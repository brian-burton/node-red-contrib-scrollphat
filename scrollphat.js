  ...
  RED.nodes.createNode(this,config);
    ...
    node.on('input', function(msg) {
      ...
      if (typeof msg.payload.x === "number" && typeof msg.payload.y === "number" && typeof msg.payload.value === "boolean") {
        ...
        //Add LEDs to existing pattern by default or reset to jsut new pixel if user specifically sets msg.payload.additive to false
        ...
        if (node.ledValue) { buffer[node.xPos] |= (1 << node.yPos); }
        else { buffer[node.xPos] &= ~(1 << node.yPos); }
      }

      //Write the entire buffer to the Scroll Phat
      scrollphat.writeI2cBlockSync(0x60, 0x01, 12, buffer);
    });
    node.on('close', function() {
      try {
        scrollphat.closeSync();
        if (scrollphat._peripherals.length == 0) { node.warn("Scroll Phat i2c connection closed."); }
